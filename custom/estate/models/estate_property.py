...
class EstateProperty(models.Model):
    ...
    state = odoo.fields.Selection(
        [('N', 'New'), ('OR', 'Offer received'), ('OA', 'Offer accepted'), ('S', 'Sold'), ('C', 'Canceled')],
        ...
        for record in self:
            if record.selling_price != 0:
                if float_compare(record.selling_price, record.expected_price * 0.9, precision_digits=3) < 0:
                    raise ValidationError('The selling price cant be lower than 90 percent of expected price')
    ...
    def unlink(self):
        for record in self:
            if record.state == 'N' or record.state == 'C':
                ...
                raise ValidationError('The property cannot be deleted only NEW or CANCELED properties can be deleted')
